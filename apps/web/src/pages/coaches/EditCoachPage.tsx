...
export default function EditCoachPage() {
  ...
  useEffect(() => {
    if (data) reset({ name: data.name, teamId: data.teamId });
  }, [data, reset]);

  const mutation = useMutation({
    mutationFn: (form: FormData) => api.patch(`/coaches/${id}`, form),
    onSuccess: () => {
  ...
  if (isError) return <FigmaErrorBanner message="Erro ao carregar técnico." />;

  return (
    <div>
      <AppPageHeader title="Editar Técnico" />
